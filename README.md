...
Load mod `.jar` files and optional resource packs, review the detected language keys, edit missing or incorrect Chinese translations, and export either a normal resource pack or patched copies of the loaded jars. The app runs in the browser, so selected files stay on your machine unless you explicitly use the optional LLM translation feature.

## What It Does

- Reads `assets/<namespace>/lang/*.json` files from Minecraft mod jars and resource packs.
- Targets `zh_cn`, `zh_tw`, and `zh_hk` translations with `en_us` as the source fallback.
